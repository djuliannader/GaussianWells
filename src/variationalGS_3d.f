...
c     $ FCT, icorr, EPSC
      ...
      COMMON / XINPUT2 / xB, EPSin, EPSid,  FCT, EPSC, llambda
...
c
      ...
      read(5,*)Bx,llambda  ! -> xB (proper units)
      read(5,*)
...
c
      ...
      write(6,*)'              the Gaussian potencial                  '
      write(6,*)
      write(6,*)'                                                      '
      write(6,*)'******************************************************'
      write(6,*)
...
c     .. COMMON BLOCK  FOR WAVE FUNCTIONS PARAMETERS ...
      COMMON / PARM / B, AA, a, h, c, d, alpha, beta, delta, lambda
      COMMON /FLAGS / iqflag
...
c
      write(*,*)
      write(*,*)
      write(*,*)' Gaussian Potential         call: ',icall
      write(*,*)'****************************************************'
      write(6,*)' Angular momentum        =',lambda
      write(6,*)' Coupling Constant       =',xB
      ...
      write(*,1010)' c       =',  c   
      write(*,1010)' d       =',  d    
      write(*,1010)' alpha   =',  alpha 
      write(*,1010)' beta    =',  beta
      write(*,1010)' AA      =',  AA       
      write(*,1010)' delta   =',  delta   
...
c     Definitions:
       ...
       phiN= (a + c*r**2)/Sqrt(1+r**2) + d*Log(1 + r**2)
c      phiN= a*r**2

c
c      La dimensión del espacio se denota por delta
      Psi0=r**l*dexp(-1.d0*phiN)
...
c
      ...
      VVa =  l**2/r**2 - (l*(-1 + r**4*(-1 + 4*d + 2*c*Sqrt(1 + r**2)) + 
     &   r**2*(-2 + 4*d - 2*a*Sqrt(1 + r**2) + 4*c*Sqrt(1 + r**2))))/(r
     &     + r**3)**2 + (a**2*r**2 + 3*a*Sqrt(1 + r**2)
     &    - 2*a*c*r**2*(2 + r**2) + c**2*r**2*(2 + r**2)**2 + 
     &   4*d**2*(r**2 + r**4) - c*Sqrt(1 + r**2)*(6 + 5*r**2 + 2*r**4) + 
     &     d*(-6 - 4*r**2*(2 + a*Sqrt(1 + r**2) - 2*c*Sqrt(1 + r**2))
     &   + r**4*(-2 + 4*c*Sqrt(1 + r**2))))/
     &   (1 + r**2)**3

c$$$      (a**2*r**2 + 3*a*Sqrt(1 + r**2) - 
c$$$     -    2*a*c*r**2*(2 + r**2) + c**2*r**2*(2 + r**2)**2 + 
c$$$     -    4*d**2*(r**2 + r**4) - 
c$$$     -    c*Sqrt(1 + r**2)*(6 + 5*r**2 + 2*r**4) + 
c$$$     -    d*(-6 - 4*r**2*(2 + a*Sqrt(1 + r**2) - 
c$$$     -          2*c*Sqrt(1 + r**2)) + 
c$$$     -       r**4*(-2 + 4*c*Sqrt(1 + r**2))))/(1 + r**2)**3
...
c     &     r**2)))))/(2.*m*r**2*(1 + r**2)**3)
       ...
       VV = -g*dexp(-r**2)+l*(l+1)/(2*r**2)

       EVARN = Psi0*(-1.0*VVa/2.0 + VV)*Psi0*r**2
c
c
      RETURN
      END
