...
c
c     --- This program minimizes the energy of the deuteron with a Gaussial well:
c                                   Ground State
c       
...
c
cc     Compile as: gfortran -o name.exe name.f
c
      PROGRAM MAIN
...
c     $ FCT, icorr, EPSC
      ...
      COMMON / XINPUT2 / aalambda, xB, EPSin, EPSid,  FCT, EPSC, llambda
...
c
      ...
      ebest  = 1.D10
c
      read(5,*)    ! Read Coupling Constant
      read(5,*)Bx,llambda, aalambda  ! -> xB (proper units)
      read(5,*)
...
c
      ...
      write(6,*)'              the Deuteron                           '
      write(6,*)
      write(6,*)'                                                      '
      write(6,*)'******************************************************'
      write(6,*)
      write(6,*)'Coupling Constant:', xB
      write(6,*)'angular momentum:', llambda
      write(6,*)'Lambda:', alambda
      write(6,*)
c
        write(*,*)'LIMITS:'
        write(*,*)'--------------------------------------------------'
...
c     .. COMMON BLOCK  FOR WAVE FUNCTIONS PARAMETERS ...
      COMMON / PARM / B, alambda, AA, a, c, d, alpha,
     &  beta, delta, lambda,a2,c2,d2,a3,c3,d3,h2,h3
      COMMON /FLAGS / iqflag
...
c
      write(*,*)
      write(*,*)
      write(*,*)' Deuteron                           call: ',icall
      write(*,*)'****************************************************'
      write(6,*)' Lambda                  =',alambda
      write(6,*)' Coupling Constant       =',xB
      write(*,*)' Variational Energy      =',FVAL,'MeVs'
      write(*,*)
      write(*,1010)' a1       =',  a
      write(*,1010)' h1       =',  1.0000      
      write(*,1010)' c1       =',  c   
      write(*,1010)' d1       =',  d    
      write(*,1010)' h2       =',  h2 
      write(*,1010)' a2       =',  a2
...
c     ELECTRON COORDINATES 
      rhoe   = XN(1)        ! This coordinate is sqrt(B)*rho_e
      Brhoe2 = rhoe*rhoe    ! This is B*rho_e**2
      rho_e  = rhoe/SQRB
      rho_   = rho_e
      rho    = rho_e
      z      = XN(2)
...
c     Definitions:  
c     ----------------------------
      gamma = B  !gammma =  Constante de Acoplamiento 
      l= lambda
c     
c     Definitions:
        g=B

c       phiN= (a + c*r**2)/Sqrt(1+r**2) + d*Log(1 + r**2)
        phiN= (a + c*(r*alambda)**2)/Sqrt(1+(r*alambda)**2)
     $       + d*Log(1 + (r*alambda)**2)

        phiN2= (a2 + c2*(r*alambda)**2)/Sqrt(1+(r*alambda)**2)
     $       + d2*Log(1 + (r*alambda)**2)

        phiN3= (a3 + c3*(r*alambda)**2)/Sqrt(1+(r*alambda)**2)
     $       + d3*Log(1 + (r*alambda)**2)


c
c      La dimensión del espacio se denota por delta
        Psi1=r**l*(dexp(-1.d0*phiN))
        Psi2=r**l*(dexp(-1.d0*phiN2))
        Psi3=r**l*(dexp(-1.d0*phiN3))

        Psi0=1.0*Psi1+h2*Psi2+h3*Psi3 
...
c
      ...
      VVa =         (alambda**2*(a**2*alambda**2*r**2
     &  + 3*a*Sqrt(1 + alambda**2*r**2) - 
     &      2*a*c*alambda**2*r**2*(2 + alambda**2*r**2) + 
     &      c**2*alambda**2*r**2*(2 + alambda**2*r**2)**2 + 
     &      4*d**2*(alambda**2*r**2 + alambda**4*r**4) - 
     &      c*Sqrt(1 + alambda**2*r**2)*
     &       (6 + 5*alambda**2*r**2 + 2*alambda**4*r**4) + 
     &      d*(-6 - 4*alambda**2*r**2*
     &          (2 + a*Sqrt(1 + alambda**2*r**2) - 
     &            2*c*Sqrt(1 + alambda**2*r**2)) + 
     &     2*alambda**4*r**4*(-1 + 2*c*Sqrt(1 + alambda**2*r**2)))))/
     &       (1 + alambda**2*r**2)**3

       VVa2 =         (alambda**2*(a2**2*alambda**2*r**2
     &  + 3*a2*Sqrt(1 + alambda**2*r**2) - 
     &      2*a2*c2*alambda**2*r**2*(2 + alambda**2*r**2) + 
     &      c2**2*alambda**2*r**2*(2 + alambda**2*r**2)**2 + 
     &      4*d2**2*(alambda**2*r**2 + alambda**4*r**4) - 
     &      c2*Sqrt(1 + alambda**2*r**2)*
     &       (6 + 5*alambda**2*r**2 + 2*alambda**4*r**4) + 
     &      d2*(-6 - 4*alambda**2*r**2*
     &          (2 + a2*Sqrt(1 + alambda**2*r**2) - 
     &            2*c2*Sqrt(1 + alambda**2*r**2)) + 
     &     2*alambda**4*r**4*(-1 + 2*c2*Sqrt(1 + alambda**2*r**2)))))/
     &     (1 + alambda**2*r**2)**3

        VVa3 =         (alambda**2*(a3**2*alambda**2*r**2
     &  + 3*a3*Sqrt(1 + alambda**2*r**2) - 
     &      2*a3*c3*alambda**2*r**2*(2 + alambda**2*r**2) + 
     &      c3**2*alambda**2*r**2*(2 + alambda**2*r**2)**2 + 
     &      4*d3**2*(alambda**2*r**2 + alambda**4*r**4) - 
     &      c3*Sqrt(1 + alambda**2*r**2)*
     &       (6 + 5*alambda**2*r**2 + 2*alambda**4*r**4) + 
     &      d3*(-6 - 4*alambda**2*r**2*
     &          (2 + a3*Sqrt(1 + alambda**2*r**2) - 
     &            2*c3*Sqrt(1 + alambda**2*r**2)) + 
     &     2*alambda**4*r**4*(-1 + 2*c3*Sqrt(1 + alambda**2*r**2)))))/
     &  (1 + alambda**2*r**2)**3

c      m=1.0

c      VVa = (2*d*r**2 - l**2*(1 + r**2)**3 + l*(1 + r**2)*(1 + r**2*(2 +
c     &     4*d - 2*(a - 2*c)*Sqrt(1 + r**2)) + r**4*(1 + 4*d +
c     & 2*c*Sqrt(1 + r**2)))+r**2*(-(a**2*r**2) - c**2*r**2*(2 + r**2)**2
c     &     - 2*d*r**2*(r**2 + 2*d*(1 + r**2)) + a*(2*c*r**2*(2 + r**2)
c     &     + Sqrt(1 + r**2)*(-1 + (2 + 4*d)*r**2)) -
c     &     c*Sqrt(1 + r**2)*(-2 + r**2*(1 + 4*d*(2 +
c     &     r**2)))))/(2.*m*r**2*(1 + r**2)**3)
      ...
      D1=0.0
      VV = (1/4.)*(3*C10+C01+1*(C10-C01)+D1)*dexp(-(alambda*r)**2/4.0)
      

c$$$      VV = -g*dexp(-r**2)+l*(l+1)/(2*r**2)
      
c      Psi0*(-1.0*g*(VVa*Psi1+h2*VVa2*Psi2+h3*VVa3*Psi3)/2.0
c     $ + 1.0*VV*Psi0)*r**2       ! 2 para Hartrees

      T=(1.0*VVa*Psi1+h2*VVa2*Psi2+h3*VVa3*Psi3)
      
      EVARN = 1.0*Psi0*(-1.0*g*T/2.0 + 1.0*VV*Psi0)*r**2
c       EVARN = Psi1*(-1.0*g*VVa/2.0 + 1.0*VV)*Psi1*r**2
c
c
      RETURN
      END
...
c
c      COORDINATES 
      rhoe   = XD(1)        ! This coordinate is sqrt(B)*rho_e
      Brhoe2 = rhoe*rhoe    ! This is B*rho_e**2
      rho_e  = rhoe/SQRB
      rho    = rho_e
c     rho_e  = rhoe
