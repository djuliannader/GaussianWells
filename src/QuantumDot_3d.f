...
c     ******************************************************************
c
c     USAGE:  Helium.e < input.dat &
c
c     --- This program minimizes the energy of the 2-dimensional Helium atom      
c         Logaritthmic Potentials Used
c      
c     --- Using MINUIT from CERN-lib and
c
c     --- D01FCF INTEGRATION SUBROUTINE  NAG-lib
c
c           Compile as: gfortran -o program.e program.f
c                       gfortran -std=legacy -o Sextic.e Sextic.f (MAC)

c 
      PROGRAM MAIN
      IMPLICIT DOUBLE PRECISION (A-H,O-Z,l)
      COMMON / NINPUT3 / nxN, nxD, nyN, nyD, nzN, nzD, icorr
      COMMON / XINPUT2 / xZ, EPSin, EPSid,  FCT, EPSC, V0
      COMMON / XINPUT4 / ao1, bo1, co1
      COMMON / XINPUT1 / XLN(5), XLD(5), YLN(10), YLD(10),  ZLN(5),
     $ ZLD(5), EPNUM(20), EPDEN(20)
...
c
      ...
      read(5,*)co1
      read(5,*)
      read(5,*)nxN, nxD  ! Number of regions in x (NUM / DEN)
      read(5,*)
      read(5,*)(XLN(ir), ir=1,nxN+1)  ! x Num limits
      read(5,*)
      read(5,*)(XLD(ir), ir=1,nxD+1)  ! x Den limits
      read(5,*)
      read(5,*)nyN, nyD     ! Number of regions in y (NUM / DEN)
      read(5,*)
      read(5,*)(YLN(ir), ir=1,nyN+1)   ! y Num limits
      read(5,*)
      read(5,*)(YLD(ir), ir=1,nyD+1)   ! y Den limits
      read(5,*)
...
c
      write(6,*)
      write(6,*)'******************************************************'
      write(6,*)'This program minimizes the variational Energy of  the '
      write(6,*)' three-dimensional Quantum Dot with Gaussian          '
      write(6,*)' confining potential                                  '
      write(6,*)'                                                      '
      write(6,*)'******************************************************'
      write(6,*)
      write(6,*)
...
c
        ...
        write(*,*)'--------------------------------------------------'
        write(*,*)
        write(*,*)(XLN(ir), ir=1,nxN+1)  !r1 Num limits
        write(*,*)
        write(*,*)(XLD(ir), ir=1,nxD+1)  !r1 Den limits
        write(*,*)
        write(*,*)'--------------------------------------------------'
        write(*,*)
        write(*,*)(YLN(ir), ir=1,nyN+1)  !r2 Num limits
        write(*,*)
        write(*,*)(YLD(ir), ir=1,nyD+1)  !r2 Den limits
        write(*,*)
        write(*,*)'--------------------------------------------------'
        write(*,*)
        write(*,*)(ZLN(ir), ir=1,nzN+1)  ! theta  Num limits
        write(*,*)
        write(*,*)(ZLD(ir), ir=1,nzD+1)  ! theta Den limits
        write(*,*)
        write(*,*)'--------------------------------------------------'
        write(*,*)
...
c     .. Parameters for the integration routine D01FCF ...
      ...
      PARAMETER       (NDIM=3,MAXPTS=1000000000,LENWRK=5000000)
c     .. LOCAL ARRAYS ...
...
c     .. COMMON BLOCK  FOR WAVE FUNCTIONS PARAMETERS ...
      COMMON / PARM /ZZ, a, b, c, c12, d12, V0p
      COMMON /FLAGS / iqflag
...
c
       ...
       IF((a**2 - c*(c12/d12) .LE.0.0D0)) then
          FVAL = 10000.0D0
          RETURN
       END IF
       

       IF((b**2 - c*(c12/d12) .LE.0.0D0)) then
          FVAL = 10000.0D0
          RETURN
       END IF        

c$$$       IF((b - c .LE.0.0D0)) then
c$$$          FVAL = 10000.0D0
c$$$          RETURN
c$$$       END IF         
...
c
      ...
      DO Iz = 1,nzN
               ...
               EPS  = EPNUM(IREG)
c
c         NUMERATOR LIMITS   (1)= x, (2)= y, (3) = z  
          ANUM(1)  = XLN(Ix)  
          BNUM(1)  = XLN(Ix+1)
          ANUM(2)  = YLN(Iy)
          BNUM(2)  = YLN(Iy+1)
...
c
      ...
      DO Iz=1,nzD
               ...
               IREG=IREG+1
               EPS  = EPDEN(IREG) 
c         LIMITS FOR THE DENOMINATOR   (1)= x, (2)= y, (3) = z
          ADEN(1)   =  XLD(Ix)  
          BDEN(1)   =  XLD(Ix+1)
          ADEN(2)   =  YLD(Iy)  
          BDEN(2)   =  YLD(Iy+1)
...
c
c     *************************
        FVAL = ZZ*(enernu/(enerde)) 
c     *************************
c     *************************
        if( FVAL .LE. ebest) then
           icallb = icall
           ebest  = FVAL
        end if
c
...
c
      write(*,*)
      write(*,*)
      write(*,*)' Quantum Dot   (3d)           : ',icall
      write(*,*)'****************************************************'
      write(6,*)' Lambda                      =', ZZ
      write(6,*)' V0                          =', V0p
      write(6,*)' numerator                   =', enernu
      write(6,*)' denominator                 =', enerde
...
c     1) DeltaX NUMER
     ...
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  x    EPS         ACC      (NUMER)    '
...
c     2) DeltaX  DENOM
     ...
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  x   EPS         ACC       (DENOM)    '
...
c     3) DeltaY  NUMER
     ...
     +  WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  y   EPS         ACC       (NUMER)    '
      write(*,5005)ABS(FINV/enerde),EPS,ACC
c
      write(*,*)' --------------------------------------------------'
c
c     4) DeltaY DENOM
     ...
     +  WRKSTR,FINV,IFAIL)
      ...
      write(*,*)
      write(*,*)'Corrections in  y   EPS         ACC       (DENOM)    '
      write(*,5005)ABS(FINV/enerde),EPS,ACC
c
      write(*,*)' --------------------------------------------------'
c
...
c     COORDINATES
      ...
      xjac = r1**2*r2**2*sin(theta2)

       r12=sqrt(r1**2+r2**2-2*r1*r2*cos(theta2))
      

       psi1 = COMPACTWF(r1,r2,r12)
       psi2 = COMPACTWF(r2,r1,r12)
      ...
      T= -(1/2.0)*Va1*psi1 -(1/2.0)*Va2*psi2

c     V = Log((r12*r13)/r23)

      V = POTENTIAL(r1,r2,r12)

      expval = r12
      

      EVARN = psi*(1.0*T+V*psi)*xjac
c      EVARN = 1.0
...
c       EVARD= 1.0
      ...
      DOUBLE PRECISION FUNCTION COMPACTWF(r1,r2,r12)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON / PARM /ZZ, a, b, c, c12, d12, V0p

       fac_exp1= EXP_FAC(a*a*r1)
       

       
       fac_exp2= EXP_FAC(b*b*r2)

       d=1.0*(1/2.0-c)

       
      var = dexp(-fac_exp1-fac_exp2)*
     &  (1+d*r12)*dexp(c*r12*(1.0+c12*r12)/(1.0+d12*r12))
      ...
      DOUBLE PRECISION FUNCTION EXP_FAC(r)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON / PARM /ZZ, a, b, c, c12, d12, V0p
      COMMON / XINPUT4 / ao1, bo1, co1


      fac_expt=(ao1+bo1*r**2)/sqrt(1+r**2) + co1*log(1+r**2)     
      ...
      DOUBLE PRECISION FUNCTION POTENTIAL(r1,r2,r3)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON / PARM /ZZ, a, b, c, c12, d12, V0p
      
      alambda= ZZ
      
      VV = - (V0p/(1.0*alambda))*(dexp(-r1**2)+dexp(-r2**2))
     &  + 1.0/(r3*sqrt(1.0*alambda)) 
      ...
      DOUBLE PRECISION FUNCTION VA(r1,r2,theta2)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON / PARM /ZZ, a, b, c, c12, d12, V0p
      COMMON / XINPUT4 / ao1, bo1, co1
...
c    Recordar que sigma -> ZZ      

      r12 = sqrt(r1**2+r2**2-2*r1*r2*cos(theta2))

      psi = COMPACTWF(r1,r2,r12)

      Vac = -((d*exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -          (c*r12*(1 + c12*r12))/
     -     (1 + d12*r12) - co1*Log(1 + a**4*r1**2)
     - - co1*Log(1 + b**4*r2**2))*
     -       (r2 - r1*Cos(theta2))**2)/(r12**2)**1.5) - 
     -     (d*exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -        (c*r12*(1 + c12*r12))/
     -     (1 + d12*r12) - co1*Log(1 + a**4*r1**2)
     - - co1*Log(1 + b**4*r2**2))*
     -     (r1 - r2*Cos(theta2))**2)/(r12**2)**1.5 + 
     -     (2*d*exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -        (c*r12*(1 + c12*r12))/
     -     (1 + d12*r12) - co1*Log(1 + a**4*r1**2)
     - - co1*Log(1 + b**4*r2**2)))/
     -   r12 + (2*d*
     -     exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -        (c*r12*(1 + c12*r12))/
     -     (1 + d12*r12) - co1*Log(1 + a**4*r1**2)
     - - co1*Log(1 + b**4*r2**2))*
     -     (r2 - r1*Cos(theta2))*((-2*b**4*co1*r2)/(1 + b**4*r2**2)
     - - (2*b**4*bo1*r2)/Sqrt(1 + b**4*r2**2) + 
     -       (b**4*r2*(ao1 + b**4*bo1*r2**2))/(1 + b**4*r2**2)**1.5 + 
     -       (c*(r2 - r1*Cos(theta2))*(1 + c12*r12))/
     -        (d12*(r1**2 + r2**2) - 2*d12*r1*r2*Cos(theta2) + r12) - 
     -       (c*d12*(r2 - r1*Cos(theta2))*(1 + c12*r12))/
     -        (1 + d12*r12)**2 + 
     -       (c*c12*(r2 - r1*Cos(theta2)))/(1 + d12*r12)))/
     -   r12 + exp
     -     (-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -      (c*r12*(1 + c12*r12))/
     -     (1 + d12*r12) - co1*Log(1 + a**4*r1**2)
     - - co1*Log(1 + b**4*r2**2))*
     -   (1 + d*r12)*
     -     ((-2*b**4*co1*r2)/(1 + b**4*r2**2)
     - - (2*b**4*bo1*r2)/Sqrt(1 + b**4*r2**2) + 
     -      (b**4*r2*(ao1 + b**4*bo1*r2**2))/(1 + b**4*r2**2)**1.5 + 
     -      (c*(r2 - r1*Cos(theta2))*(1 + c12*r12))/
     -       (d12*(r1**2 + r2**2) - 2*d12*r1*r2*Cos(theta2) + r12) - 
     -      (c*d12*(r2 - r1*Cos(theta2))*(1 + c12*r12))/
     -       (1 + d12*r12)**2 + 
     -      (c*c12*(r2 - r1*Cos(theta2)))/(1 + d12*r12))**2 + 
     -     (2*d*exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -        (c*r12*(1 + c12*r12))/
     - (1+d12*r12)-co1*Log(1 + a**4*r1**2) - co1*Log(1 + b**4*r2**2))*
     -     (r1 - r2*Cos(theta2))*((-2*a**4*co1*r1)/(1 + a**4*r1**2)
     - - (2*a**4*bo1*r1)/Sqrt(1 + a**4*r1**2) + 
     -       (a**4*r1*(ao1 + a**4*bo1*r1**2))/(1 + a**4*r1**2)**1.5 + 
     -       (c*(r1 - r2*Cos(theta2))*(1 + c12*r12))/
     -        (d12*(r1**2 + r2**2) - 2*d12*r1*r2*Cos(theta2) + r12) - 
     -       (c*d12*(r1 - r2*Cos(theta2))*(1 + c12*r12))/
     -        (1 + d12*r12)**2 + 
     -       (c*c12*(r1 - r2*Cos(theta2)))/(1 + d12*r12)))/
     -   r12 + exp
     -     (-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -      (c*r12*(1 + c12*r12))/
     ...
     -      (c*d12*(r1 - r2*Cos(theta2))*(1 + c12*r12))/
     -       (1 + d12*r12)**2 + 
     -      (c*c12*(r1 - r2*Cos(theta2)))/(1 + d12*r12))**2 + 
     -     exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -      (c*r12*(1 + c12*r12))/
     - (1 + d12*r12)-co1*Log(1+a**4*r1**2) - co1*Log(1 + b**4*r2**2))*
     -   (1 + d*r12)*
     -     ((4*b**8*co1*r2**2)/(1 + b**4*r2**2)**2 + (4*b**8*bo1*r2**2)
     - /(1 + b**4*r2**2)**1.5 - (2*b**4*co1)/(1 + b**4*r2**2) - 
     -     (2*b**4*bo1)/Sqrt(1 + b**4*r2**2) - (3*b**8*r2**2*(ao1
     - + b**4*bo1*r2**2))/(1 + b**4*r2**2)**2.5 + 
     -     (b**4*(ao1 + b**4*bo1*r2**2))/(1 + b**4*r2**2)**1.5 + 
     -     (c*(1 + c12*r12))/
     -      (d12*(r1**2 + r2**2) - 2*d12*r1*r2*Cos(theta2) + r12) + 
     -     (2*c*d12**2*(r2 - r1*Cos(theta2))**2*(1 + c12*r12))/
     -      (r12*(1 + d12*r12)**3) - 
     -     (2*c*c12*d12*(r2 - r1*Cos(theta2))**2)/
     -      (r12*(1 + d12*r12)**2) - 
     -     (c*d12*(1 + c12*r12))/(1 + d12*r12)**2 - 
     -     (c*d12*(r2 - r1*Cos(theta2))**2*(1 + c12*r12))/
     -      ((r12**2)*(1 + d12*r12)**2) + 
     -     (c*c12)/(1 + d12*r12) + 
     -     (c*c12*(r2 - r1*Cos(theta2))**2)/
     -      ((r12**2)*(1 + d12*r12)) - 
     -     (c*(r2 - r1*Cos(theta2))**2*(1 + c12*r12))/
     -      ((r12**2)**1.5*(1 + d12*r12))) + 
     -     exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -      (c*r12*(1 + c12*r12))/
     -  (1+d12*r12)-co1*Log(1 + a**4*r1**2) - co1*Log(1 + b**4*r2**2))*
     -   (1 + d*r12)*
     -     ((4*a**8*co1*r1**2)/(1 + a**4*r1**2)**2 + (4*a**8*bo1*r1**2)/
     - (1 + a**4*r1**2)**1.5 - (2*a**4*co1)/(1 + a**4*r1**2) - 
     -     (2*a**4*bo1)/Sqrt(1 + a**4*r1**2) - (3*a**8*r1**2*(ao1
     - + a**4*bo1*r1**2))/(1 + a**4*r1**2)**2.5 + 
     -     (a**4*(ao1 + a**4*bo1*r1**2))/(1 + a**4*r1**2)**1.5 + 
     -     (c*(1 + c12*r12))/
     -      (d12*(r1**2 + r2**2) - 2*d12*r1*r2*Cos(theta2) + r12) + 
     -     (2*c*d12**2*(r1 - r2*Cos(theta2))**2*(1 + c12*r12))/
     -      (r12*(1 + d12*r12)**3) - 
     -     (2*c*c12*d12*(r1 - r2*Cos(theta2))**2)/
     -      (r12*(1 + d12*r12)**2) - 
     -     (c*d12*(1 + c12*r12))/(1 + d12*r12)**2 - 
     -     (c*d12*(r1 - r2*Cos(theta2))**2*(1 + c12*r12))/
     -      ((r12**2)*(1 + d12*r12)**2) + 
     -     (c*c12)/(1 + d12*r12) + 
     -     (c*c12*(r1 - r2*Cos(theta2))**2)/
     -      ((r12**2)*(1 + d12*r12)) - 
     -     (c*(r1 - r2*Cos(theta2))**2*(1 + c12*r12))/
     -      ((r12**2)**1.5*(1 + d12*r12))) + 
     -     (exp(-(ao1/Sqrt(1 + a**4*r1**2)) - (a**4*bo1*r1**2)/
     -  Sqrt(1 + a**4*r1**2) - ao1/Sqrt(1 + b**4*r2**2) - 
     -        (b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -        (c*r12*(1 + c12*r12))/
     -         (1 + d12*r12))*
     -     ((2*d*(r1 - r2*Cos(theta2)))/r12 + 
     -       2*(1 + d*r12)*
     -     ((-2*a**4*co1*r1)/(1 + a**4*r1**2)
     - - (2*a**4*bo1*r1)/Sqrt(1 + a**4*r1**2) + 
     -          (a**4*r1*(ao1 + a**4*bo1*r1**2))/(1 + a**4*r1**2)**1.5 + 
     ...
     -           (1 + d12*r12)**2 + 
     -          (c*c12*(r1 - r2*Cos(theta2)))/(1 + d12*r12))))/
     -   (r1*(1 + a**4*r1**2)**co1*(1 + b**4*r2**2)**co1) + 
     -     (exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     - - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -        (c*r12*(1 + c12*r12))/
     -     (1 + d12*r12) - co1*Log(1 + a**4*r1**2)
     -  - co1*Log(1 + b**4*r2**2))*
     -     (2*d*(1 + b**4*r2**2)**1.5*(r2 - r1*Cos(theta2))*
     - (1 + d12*r12)**2 - 
     -       (1 + d*r12)*
     -        (2*c*d12*(1 + b**4*r2**2)**1.5*(r2 - r1*Cos(theta2))*r12*
     -           (1 + c12*r12) - 
     -          2*c*c12*(1 + b**4*r2**2)**1.5*(r2 - r1*Cos(theta2))*r12*
     -           (1 + d12*r12) - 
     -   2*c*(1 + b**4*r2**2)**1.5*(r2 - r1*Cos(theta2))*(1 + c12*r12)*
     -           (1 + d12*r12) + 
     -          4*b**4*co1*r2*Sqrt(1 + b**4*r2**2)*r12*
     -           (1 + d12*r12)**2 + 
     -          4*b**4*bo1*r2*(1 + b**4*r2**2)*r12*
     -           (1 + d12*r12)**2 - 
     -          2*b**4*r2*(ao1 + b**4*bo1*r2**2)*r12*
     -           (1 + d12*r12)**2) + 
     -     2*r1*(1 + b**4*r2**2)**1.5*Cos(theta2)*
     -   (c + d + d*d12**2*(r1**2 + r2**2) + 
     -          c*d*r12 + 2*d*d12*r12 + 
     -          c*c12*(d12*(r1**2 + r2**2) + 2*r12 + 
     -             d*(r1**2 + r2**2)*(2 + d12*r12)) - 
     -     2*r1*r2*Cos(theta2)*(d*d12**2
     -  + c*c12*(d12 + d*(2 + d12*r12))))))/
     -   (2.*r2*(1 + b**4*r2**2)**1.5*r12*
     -     (1 + d12*r12)**2) + 
     -     (exp(-((ao1 + a**4*bo1*r1**2)/Sqrt(1 + a**4*r1**2))
     -   - (ao1 + b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2) + 
     -        (c*r12*(1 + c12*r12))/
     -     (1 + d12*r12) - co1*Log(1 + a**4*r1**2)
     - - co1*Log(1 + b**4*r2**2))*
     -     (2*d*(1 + b**4*r2**2)**1.5*(r2 - r1*Cos(theta2))*(r12**2)*
     -        (1 + d12*r12)**4 - 
     -       (r12**2)*(1 + d*r12)*
     -        (1 + d12*r12)**2*
     -        (2*c*d12*(1 + b**4*r2**2)**1.5*(r2 - r1*Cos(theta2))*r12*
     ...
     -          2*b**4*r2*(ao1 + b**4*bo1*r2**2)*r12*
     -           (1 + d12*r12)**2) + 
     -       2*r1*(1 + b**4*r2**2)**1.5*(d*Cos(theta2)*(r12**2)*
     -           (1 + d12*r12)**4 - 
     -          d*r1*r2*(1 + d12*r12)**4*Sin(theta2)**2 + 
     -          2*c*d*r1*r2*r12*(1 + d12*r12)**2*
     -     (1 - 2*c12*d12*r1*r2*Cos(theta2)
     -  + c12*(d12*r1**2 + d12*r2**2 + 2*r12))*
     -           Sin(theta2)**2 + c**2*r1*r2*r12*
     -           (1 + d*r12)*
     -     (1 - 2*c12*d12*r1*r2*Cos(theta2)
     -   + c12*(d12*r1**2 + d12*r2**2 + 2*r12))**2*
     -           Sin(theta2)**2 - c*(1 + d*r12)*
     -           (1 + d12*r12)*
     -           (-4*c12*d12*r1**2*r2**2*Cos(theta2)**3*(3 + d12*r12) + 
     -             2*r1*r2*Cos(theta2)**2*(1 + d12*r12 + 
     -                2*c12*(3*d12*(r1**2 + r2**2) + r12 + 
     -                   d12**2*(r1**2 + r2**2)*r12)) + 
     -             r1*r2*(1 + 3*d12*r12 + 
     -        c12*d12*(r1**2 + r2**2)*(3 + d12*r12))*Sin(theta2)**2 - 
     -             Cos(theta2)*((r1**2 + r2**2)*(1 + d12*r12 + 
     -                   c12*(3*d12*(r1**2 + r2**2) + 2*r12 + 
     -                      d12**2*(r1**2 + r2**2)*r12)) + 
     -      2*c12*d12*r1**2*r2**2*(3 + d12*r12)*Sin(theta2)**2)))))/
     -   (2.*r2*(1 + b**4*r2**2)**1.5*(r12**2)**1.5*
     -     (1 + d12*r12)**4)

c$$$      (exp(-(ao1/Sqrt(1 + a**4*r1**2)) - (a**4*bo1*r1**2)
c$$$     - /Sqrt(1 + a**4*r1**2) - ao1/Sqrt(1 + b**4*r2**2) - 
c$$$     -     (b**4*bo1*r2**2)/Sqrt(1 + b**4*r2**2))*((4.*a**8*co1*r1**2)
c$$$     - /(1 + a**4*r1**2)**2 + (2.*a**4*ao1)/(1 + a**4*r1**2)**1.5 + 
c$$$     -     (6.*a**8*bo1*r1**2)/(1 + a**4*r1**2)**1.5 - (6.*a**4*co1)
c$$$     - /(1 + a**4*r1**2) - (6.*a**4*bo1)/Sqrt(1 + a**4*r1**2) - 
c$$$     -     (3.*a**8*r1**2*(ao1 + a**4*bo1*r1**2))/(1 + a**4*r1**2)**2.5
c$$$     - + (1.*a**4*(ao1 + a**4*bo1*r1**2))/(1 + a**4*r1**2)**1.5 + 
c$$$     -     (1.*a**8*r1**2*(-1.*ao1 + 2.*co1*Sqrt(1 + a**4*r1**2)
c$$$     - + bo1*(2. + a**4*r1**2))**2)/(1. + a**4*r1**2)**3 + 
c$$$     -     (4.*b**8*co1*r2**2)/(1 + b**4*r2**2)**2 + (2.*ao1*b**4)
c$$$     - /(1 + b**4*r2**2)**1.5 + 
c$$$     -     (6.*b**8*bo1*r2**2)/(1 + b**4*r2**2)**1.5 - (6.*b**4*co1)
c$$$     - /(1 + b**4*r2**2) - (6.*b**4*bo1)/Sqrt(1 + b**4*r2**2) - 
c$$$     -     (3.*b**8*r2**2*(ao1 + b**4*bo1*r2**2))/(1 + b**4*r2**2)**2.5
c$$$     - + (1.*b**4*(ao1 + b**4*bo1*r2**2))/(1 + b**4*r2**2)**1.5 + 
c$$$     -     (1.*b**8*r2**2*(-1.*ao1 + 2.*co1*Sqrt(1 + b**4*r2**2)
c$$$     - + bo1*(2. + b**4*r2**2))**2)/(1. + b**4*r2**2)**3))/
c$$$     -  ((1 + a**4*r1**2)**co1*(1 + b**4*r2**2)**co1)
